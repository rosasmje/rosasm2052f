...
FillCodeSymbols:
    ...
    mov ecx D$LocalLabelListA | add ecx 4
...
;;
    ...
    push esi
L1:   lodsb | cmp al EOI | ja L1<
      add esi 4 | lodsd | mov D$StatementsCounter eax
...
L3:             inc esi | cmp B$esi EOI | ja L3< | inc esi
                ...
                add edi 3 | mov D$LastRefLLListPos ecx
            .Else
...
BuildPlainLabelList:
    ...
    .While esi < edx
        cmp B$esi+2 EOI | jne L1>
            ...
            ; |L0|dWord Byte| >>> 9
            add esi 9 | inc ecx | jmp L2>
L1:     inc ebx
L0:     lodsb | stosb | cmp al EOI | ja L0<
        movsd   ; Ptr
        movsw   ; Flag |
L2: .End_While
    mov D$PlainLabelNum ebx, D$LocalLabelNum ecx
...
____________________________________________________________________________________________
; D$ size, ['L1', EOI, Flag, D$ CodePtr, LabelListPtr] = 12 ; thus 3xDword struct
[LocalLabelListA: ?    EndOfLocalLabelListA: ?]
BuildLocalLabelListA:
    VirtualFree D$LocalLabelListA
    mov esi D$LabelList, eax D$LocalLabelNum
    lea eax D$eax*2+eax | shl eax 2 | add eax 4
    VirtualAlloc LocalLabelListA eax

    mov edx esi | add edx D$edx | add esi 5
    mov edi D$LocalLabelListA | add edi 4 ; keeps 4-align
    ...
    .While esi < edx
        cmp B$esi+2 EOI | jne L1>
            ...
            cmp ah '9' | ja L1> | cmp ah '0' | jb L1>
            cmp al 'A' | jb L1> | cmp al 'Z' | ja L1>
            stosd | mov eax esi | add esi 3 | movsd | stosd | lodsw
            mov B$edi-9 al | jmp L2>

L1:     lodsb | cmp al EOI | ja L1< | add esi 6 ; skip
L2: .End_While

    mov D$EndOfLocalLabelListA edi, eax D$LocalLabelListA
    sub edi eax | mov D$eax edi
ret
____________________________________________________________________________________________
; EAX = 'L1<' EDX = RefPtr ECX=LastRefLLListPos
[LastRefLLListPos: ?]
FindNearestLocalLabelInLLListA:
    push ebx, esi
    mov edi D$LocalLabelListA | mov esi D$edi | add esi edi | add edi 4
    mov ebx eax | shr ebx 16 | cmp bl '<' | mov ebx edi | mov edi ecx | jne L1> ; goto Down

L0: cmp D$edi+4 edx | ja L0> | add edi 12 | cmp edi esi | jb L0<
L0: sub edi 12 | mov ecx edi
L0: cmp W$edi ax | je L7> | sub edi 12 | cmp ebx edi | jbe L0< | jmp L8>
L1:
L0: cmp D$edi+4 edx | ja L0> | add edi 12 | cmp edi esi | jb L0< | jmp L8>
L0: mov ecx edi
L0: cmp W$edi ax | je L7> | add edi 12 | cmp edi esi | jb L0< | jmp L8>

L7: mov edi D$edi+8 | sub eax eax | jmp L9>
L8: or eax 0-1
L9: pop esi ebx
ret
____________________________________________________________________________________________
